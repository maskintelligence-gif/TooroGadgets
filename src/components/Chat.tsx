  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 h-[calc(100vh-140px)] flex flex-col">
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">We typically reply within a few minutes.</p>
      </div>
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                ...
                <p className={`text-[10px] mt-1 ${msg.isUser ? 'text-blue-100' : 'text-gray-400'}`}>
                  {msg.time}
                </p>
              </div>
            </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            />
            <button
              onClick={handleSend}
